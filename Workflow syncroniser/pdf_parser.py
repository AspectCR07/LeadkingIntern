...
def parse_checklist(filepath):
    ...
    # MAWB No & Date
    mawb_match = re.search(r"Mawb\s*No\s*&\s*Date\s*:\s*(.*?)(?=Total\s*Drawback\s*Amt|Bank\s*A/C\s*No|$|\n)", full_text, re.IGNORECASE)
    if mawb_match:
        data["mawb_no_date"] = mawb_match.group(1).strip()
    else:
        data["mawb_no_date"] = ""

    # HAWB No & Date
    hawb_match = re.search(r"Hawb\s*No\s*&\s*Date\s*:\s*(.*?)(?=Total\s*RODTEP\s*Amt|Dbk\s*Bank\s*A/C|$|\n)", full_text, re.IGNORECASE)
    if hawb_match:
        data["hawb_no_date"] = hawb_match.group(1).strip()
    else:
        data["hawb_no_date"] = ""

    # Chargeable Weight
    chg_wt_match = re.search(r"Chargeable\s*Weight\s*:\s*([\d\.,]+)", full_text, re.IGNORECASE)
    if chg_wt_match:
        data["chargeable_weight"] = f"{chg_wt_match.group(1).strip()} KGS"
    else:
        data["chargeable_weight"] = ""

    # AD Code
    ad_match = re.search(r"AD\s*Code\s*:\s*([^\n\s]+)", full_text, re.IGNORECASE)
    if ad_match:
        data["ad_code"] = ad_match.group(1).strip()
    else:
        data["ad_code"] = ""

    # GSTN
    gstn_match = re.search(r"GSTN\s*Type\s*/\s*ID\s*:\s*(?:GSN\s*/|GST\s*/)?\s*([^\n\s]+)", full_text, re.IGNORECASE)
    if gstn_match:
        data["gstn"] = gstn_match.group(1).strip()
    else:
        data["gstn"] = ""
