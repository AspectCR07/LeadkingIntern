...
namespace EasyQuoteImport
{
    public partial class MainWindow : Window
    {
        ...
        public class ColumnDef
        {
            ...
            public bool IsChennai { get; set; } = true; // true = Green Theme (LCL), false = Blue Theme (FCL)
        ...
        private void InitializeData()
        {
            ...
            // Define initial columns: 40 FCL, 20FCL, LCL
            columnDefs = new List<ColumnDef>
            {
                new ColumnDef { Header = "40 FCL", IsEnabled = true, IsChennai = false },
                new ColumnDef { Header = "20FCL", IsEnabled = true, IsChennai = false },
                new ColumnDef { Header = "LCL", IsEnabled = true, IsChennai = true }
            };

            // Define initial rows for import charges and load their default/current values
            var initialRows = new List<(string Name, string[] Values)>
            {
                ("Customs Duty", new string[] { "At Actuals", "At Actuals", "At Actuals" }),
                ("Liner Delivery order charges", new string[] { "At Actuals", "At Actuals", "At Actuals" }),
                ("CFS Charges", new string[] { "At Actuals", "At Actuals", "At Actuals" }),
                ("Documentation", new string[] { "Rs.1500/- per Bill", "Rs.1500/- per Bill", "Rs.1500/- per Bill" }),
                ("Handling Charges", new string[] { "Rs.3500/- per 40ft", "Rs.2500/- per 20ft", "Rs.1200/- per LCL" }),
                ("Delivery expenses", new string[] { "At Actuals", "At Actuals", "At Actuals" }),
                ("Service Charges", new string[] { "Rs.3000/- per 40ft", "Rs.2500/- per 20ft", "Rs.1800/- per LCL" }),
                ("Container Transportation", new string[] { "Rs.10500/- per 40ft", "Rs.9500/- per 20ft", "N.A." }),
                ("Cargo Transportation", new string[] { "N.A.", "N.A.", "Rs.3500/- per LCL" }),
                ("Loading Charges", new string[] { "Rs.1200/- per 40ft", "Rs.800/- per 20ft", "Rs.500/- per LCL" }),
                ("PQ", new string[] { "At Actuals", "At Actuals", "At Actuals" }),
                ("Fumigation", new string[] { "Rs.1500/-", "Rs.1200/-", "Will advise" })
            };
        ...
        private void BuildGridWorkspace()
        {
            ...
            for (int colIdx = 0; colIdx < columnDefs.Count; colIdx++)
            {
                ...
                string themeTooltip = col.IsChennai ? "Style: Green Theme (LCL). Click to change to Blue." : "Style: Blue Theme (FCL). Click to change to Green.";
            ...
            // Column headers (now as TextBoxes for editing!)
            for (int colIdx = 0; colIdx < columnDefs.Count; colIdx++)
            {
                var col = columnDefs[colIdx];
                int colGridPos = colIdx + 1;
            ...
            for (int rowIdx = 0; rowIdx < rowDefs.Count; rowIdx++)
            {
                ...
                rowTbx.GotFocus += (s, e) => rowTbx.BorderBrush = new SolidColorBrush((Color)ColorConverter.ConvertFromString("#4F46E5"));
                ...
                for (int colIdx = 0; colIdx < columnDefs.Count; colIdx++)
                {
                    ...
                    string fgColorStr = col.IsChennai ? "#16A34A" : "#4F46E5"; // Green for LCL, Indigo/Blue for FCL
                    ...
                    tbx.GotFocus += (s, e) => tbx.BorderBrush = new SolidColorBrush((Color)ColorConverter.ConvertFromString("#4F46E5"));
        ...
        private void AddColumnBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            var newCol = new ColumnDef
            {
                Header = "New Column",
                IsEnabled = true,
                IsChennai = false // Default to FCL/Blue theme
            };
        ...
        private string GenerateHtmlTable()
        {
            ...
            // Main Quote Table (Using dark slate indigo frame)
            sb.Append("<table style=\"border-collapse: collapse; width: 100%; font-family: 'Calibri', 'Arial', sans-serif; font-size: 13px; border: 2px solid #1e1b4b;\">");

            // --- Headers Row ---
            sb.Append("<thead><tr style=\"background-color: #1e1b4b; color: #ffffff;\">");
            sb.Append("<th style=\"border: 1px solid #cbd5e1; padding: 10px 12px; text-align: left; background-color: #1e1b4b; font-weight: bold; font-size: 13px;\">Charges Description</th>");
                ...
                if (row.IsEnabled)
                {
                    ...
                    for (int colIdx = 0; colIdx < columnDefs.Count; colIdx++)
                    {
                        ...
                        if (col.IsEnabled)
                        {
                            ...
                            string cellFgColor = col.IsChennai ? "#2e7d32" : "#4f46e5";
        ...
        private void SaveHtmlFileBtn_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var sfd = new SaveFileDialog
                {
                    ...
                    FileName = $"Import_Quotation_{ClientNameTxt.Text.Replace(" ", "_")}.html",
                    Title = "Export Import Quotation to HTML File"
                };
...
<head>
    <meta charset=""utf-8"">
    <title>Import Quotation - {ClientNameTxt.Text}</title>
    <style>
        body {{
            padding: 30px;
...
<body>
    <div class=""card"">
        ...
        private void GenerateExcelWorkbook(string filePath)
        {
            ...
            {
                var ws = workbook.Worksheets.Add("Import Quotation");

                ws.Style.Font.FontName = "Calibri";
                ws.Style.Font.FontSize = 11;

                // 1. Title Block (Header)
                ws.Cell("A1").Value = "IMPORT SHIPPING QUOTATION";
                ws.Cell("A1").Style.Font.Bold = true;
                ws.Cell("A1").Style.Font.FontSize = 16;
                ws.Cell("A1").Style.Font.FontColor = XLColor.FromHtml("#1E1B4B");
                ...
                ws.Cell(currentExcelRow, 1).Style.Fill.BackgroundColor = XLColor.FromHtml("#1E1B4B");
                ...
                for (int colIdx = 0; colIdx < columnDefs.Count; colIdx++)
                {
                    ...
                    if (col.IsEnabled)
                    {
                        ...
                        string colBgColor = col.IsChennai ? "#E2F0D9" : "#DDEBF7";
                        string colFgColor = col.IsChennai ? "#2E7D32" : "#4F46E5";
                ...
                tableRange.Style.Border.OutsideBorderColor = XLColor.FromHtml("#1E1B4B");
